...
void MAX30105_softReset(void) {
  ...
  softReset_baseline = chVTGetSystemTimeX();	

  while ( ST2MS(chVTTimeElapsedSinceX(softReset_baseline)) < 100)
  {
...
float MAX30105_readTemperature() {
  ...
  temperature_baseline = chVTGetSystemTimeX();	

  while ( ST2MS(chVTTimeElapsedSinceX(temperature_baseline)) < 100)
  {
...
uint8_t MAX30105_safeCheck(uint8_t maxTimeToCheck)
{
  ...
  safeCheck_baseline = chVTGetSystemTimeX();	 
  while(1)
  {
